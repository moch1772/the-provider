...
<?php
    ...
    $auther = mysqli_query($conn, "SELECT ID FROM user where name='$search'");
    while($rows = mysqli_fetch_array($auther))
    {   
        $result = mysqli_query($conn, "SELECT * FROM post where userID=".$rows['ID']."");
        while($row = mysqli_fetch_array($result))
        {?>
            <tr>
            <td><?php echo $row['postID']; ?></td>
